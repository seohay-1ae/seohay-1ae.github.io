function Spider() {
  ...
  for (
    var i = 0, xL = 0.05 * this.COL_WIDTH;
    i < GAME.N_COLUMNS;
    i++, xL += this.COL_WIDTH
  ) {
  ...
  this.REMAINDER = [];
  this.ACE_COLUMN = new Column(
    this.COL_WIDTH * 10 + 10,
...
Spider.prototype.draw = function () {
  ...
  var w = GAME.cnv.width * 0.4,
    h = GAME.cnv.height * 0.4;
  ...
  if (this.first) {
    GAME.g2.fillStyle = "#ff0";
    GAME.g2.fillText("스파이더 솔리테어", w, h - 80);
    // GAME.g2.fillText("12 cards per suit, no kings", w, h - 40);
    GAME.g2.fillText("Ctrl+Z 되돌리기", w, h);
  }
  this.first = false;
};

Target.prototype.isRemainder = function () {
  return this.column === GAME.game.REMAINDER;
};
...
function onMousedown(e) {
  ...
  var nextFaceUp = false,
    i1 = card.columnIndex - 1;
...
function moveTo(column) {
  ...
  if (column.endsWithFullSuit()) {
    ...
    var targetYT =
      Card.prototype.height + 50 + column.DY * GAME.game.nCompleted;
    var nSteps = 120;
    for (var i = cards.length - 1; i >= 0; i--) {
      var card = cards[i];
      card.XL = column.XL;
